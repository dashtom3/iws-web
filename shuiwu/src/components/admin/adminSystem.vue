...
export default {
  ...
  created () {
    ...
    // 请求图片列表
    axios.post(global.baseUrl + 'system/pic?token=' + localStorage.token)
    .then((res) => {
      // console.log(res)
  ...
  methods: {
    ...
    del (id) {
      var self = this
      axios.post(global.baseUrl + 'system/delete?token=' + localStorage.token + '&systemId=' + id)
      .then((res) => {
    ...
    addSystem () {
      ...
      axios.post(global.baseUrl + 'system/add?token=' + localStorage.token, systemMsg)
      .then((res) => {
        console.log(res)
        if (res.data.callStatus === 'SUCCEED') {
